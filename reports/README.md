...
## LLM Inference Research & Safety Alignment — ~1,348,000 Primary + Judge Measurements + the TR164 cross-version data-lineage reconciliation (2026-06-24), 49+ Technical Reports

**Latest (2026-06-24):** **TR164 cross-version data-lineage reconciliation + count fix.** Built `research/tr164/TR164_DATA_LINEAGE.md` — the canonical run-dir → version → rows → finding → methodology-TR → paper-section map for the whole TR164 program — and appended a grounded *Data Reconciliation & Cross-Version Lineage* section to all five version reports (V1–V5), each tracing its exact data provenance and the work built after it (weight-axis matrix, closure audits, 70B confirmation, policy-cap validation, TR170 pilot). The reconciliation audit **caught a count gap**: the TR165 same-session `PYTHON_GIL=1`/`PYTHON_GIL=0` control arms (5,184 rows, runs `20260623_165503` / `20260623_222630` — the SSP submission's same-build GIL control) were uncounted in *both* the 2026-06-08 and the initial 2026-06-24 supplement; folding them in moves the program total **~1,337,000 → ~1,348,000** (full 2026-06-24 supplement = +11,781: 6,042 TR164 SSP substrate + 5,184 TR165 GIL-control arms + 555 TR170 pilot). Exposure-clean (zero hits on the mandatory pre-push grep). **Previous (2026-06-22):** **TR164 V5 — Evidence Closure for Serving-Stack Physics** hand-promoted (PI directive). `Technical_Report_164_V5.md` adds three reviewer-facing closure layers to the V4 amortization model: (1) an explicit predictor-baseline + failure-mode audit over the 672-cell static grid (96 curves), (2) SGLang served validation of the static-knee-as-upper-bound claim (**8/8 ok cells**), and (3) **cross-family true-decode weight-axis matrices on a single H100** for Qwen2.5, Mistral, and Gemma 3 — the reviewer-facing answer to "the 7–8B grid barely moves W." **The cleanest positive evidence is now cross-family:** on within-family pairs built so $Ck/W$ and a KV-traffic-only baseline predict *opposite* knee directions, the larger model knees later in **Qwen2.5 14B→32B (3/3 broad + 2/2 confirm)**, **Mistral 7B→24B (3/3)**, and **Gemma 3 12B→27B (3/3)** (per-family Spearman 0.9524 / 1.000 / 0.9167; pooled cross-family **ρ = 0.8142, bootstrap 95% CI [0.57, 0.92]**, n = 24 cells / 23 finite, LOO jackknife [0.79, 0.85]). The predictor audit confirms **$Ck/W$ + a per-GPU α is the best static-grid predictor** (finite-only Spearman 0.8779, censored-as-128 0.8462), context-only and KV-only both weaker. **The closure also narrows the strongest claim, honestly:** Gemma 3 is non-monotone across all three sizes — its 4B→12B contrast inverts in **0/3** (weight and KV bytes/token both grow ≈2.8×, leaving $Ck/W$ near-degenerate and non-discriminating) and 4B→27B is mixed at 2/3, so V5 does **not** claim a universal "larger model → later knee" law. Two no-new-GPU decode-closure audits resolve the V4 prefill confound: the static grid times full generation (prefill is *inside* the timed window), but a paired marginal-decode (D512−D64) surface and a streaming true-decode audit that excludes first-token/prefill time **both preserve the rank result** (streaming finite ρ = 0.886; the zero-parameter magnitude factor tightens 2.022 → 1.322 on the clean decode surface), while a paired bootstrap shows the $Ck/W$-vs-context-only margin is **competitive, not dominant** (Δρ = 0.026, 95% CI [−0.073, 0.122] spans zero). All numbers trace to `research/tr164/v5/v5_predictor_baselines.json`, `weight_axis_decode/weight_axis_decode_summary.json`, `vllm_decode_timed_closure/decode_timed_summary.json`, and `mlsys_closure/predictor_baselines_extended.json`; deep per-experiment methodology for all eight TR164 experiments is captured in `research/tr164/methodology/TR_*.md` (full-depth follow-up reference set + claim→TR index). The companion `papers/serving_stack_physics/tmlr_whole_draft/` integrates the closure + cross-family matrix and passes a six-agent pre-submission readiness audit. Exposure-clean (zero hits on the mandatory pre-push grep). **Previous (2026-06-17):** **TR164 V4 — The Amortization-Breakdown Surface: a predictive bandwidth model for when continuous batching stops paying** hand-promoted (PI directive). `Technical_Report_164_V4.md` (**~24.3K words / 38 numbered sections / 54 subsections** full-depth, 7 figures, plus agent-reference appendices F–I) is the offline static-batch complement to the V3 closed-loop boundary matrix. Substrate: a **672-cell** Modal cross-product — 3 instruction-tuned 7–8B models (qwen2.5-7b, llama3.1-8b, mistral-7b) × 2 datacenter GPUs (**A100-80GB + H100**) × 2 decode lengths (64, 512) × {fp16, fp8-weight} × 4 contexts (512–32,000 exact tokens) × 7 batches (1–64), 3 reps, **`ok_rate` = 1.0 (672/672, 2,016 timed decode generations)**, vLLM 0.10.2 V1. **The load-bearing finding is a predictive mechanism, not a description:** a first-principles bandwidth model **η(B) = (1+r)/(1+Br), r = C·k/W** (per-request KV read bytes over weight bytes) fits the 96 measured efficiency curves at **median R² = 0.93** and predicts the observed breakdown knee from **model architecture alone (zero fitted parameters) at Spearman ρ = 0.84**, deviating only by a GPU-specific batch-saturation factor **α (0.52 A100, 0.33 H100)** that a first-principles roofline check (`derive_alpha.py`) shows is **neither compute overlap** (decode is 4.6%-compute, memory-bound throughout) **nor a VRAM-capacity effect at the knee** (95% of knees sit at ~¼ of the VRAM concurrency ceiling, where KV still fits) but an empirical, hardware-specific effect of achieved memory-bandwidth utilization rising with batch (MBU 0.55→saturation). Every axis sign falls out of that one KV-footprint scaling (the knee scales as 1/(C·k), ρ=0.84), via paired Wilcoxon + Holm-Bonferroni + percentile bootstrap CI (seed 164, 10,000 resamples): **context dominates** (mean η 0.865→0.599 from ctx512→ctx32000, paired Δ = −0.266 [−0.294, −0.240], factorial-ANOVA partial η² 0.27 vs gpu 0.038 / precision 0.018 / model 0.009); the **H100 sustains η +0.077** [0.067, 0.088] with an advantage that **grows with context** (difference-in-differences +0.088, Holm p < 1e-5); **fp8 weight-quant is −0.054 less batch-robust** [−0.061, −0.047] but at a **context-invariant** penalty (interaction null, Holm p = 0.88, correcting the intuition it should escalate); and **KV-bytes-per-token predicts the cross-model order** — Qwen (57,344 B/tok via 28 layers × 4 KV heads) knees at batch 16 at 32K context vs Llama/Mistral (131,072 B/tok) at batch 8, with the architecturally identical Llama and Mistral a tie (Holm p = 0.13). Decode is confirmed **memory-bandwidth-bound** (measured MBU ≈ 0.54–0.66 at batch 1); the aggregate-throughput speedup ceiling at batch 64 falls **42× → 25×** across the context range; measurement reliability is high (per-cell CV median 0.4%). The report includes the full **methodology-honest execution arc** (§8): the evidence-based **V0-vs-V1 engine rejection** (V0 returned 19 tok/s vs V1's 1,182 at the stressed corner — a 62× gap), the fp8-axis scoping to weight-quant, a validate-before-pay ten-cell diagnostic, an **eleven-container orphan-billing catch**, and dual-channel `modal.Dict` persistence proven to agree at 672 cells. The four cross-cuts and the architectural knee table (24/24 verified) are reproduced in-band; **all numbers trace to `research/tr164/amortization_full_stats.json` + `amortization_stats.json`** (analyzers `analyze_amortization{,_full}.py`; figures in `papers/serving_stack_physics/figures/`). **A companion closed-loop grid (§24, `modal_closed_loop_sweep.py` → `served_knee_stats.json`) now answers the "does it hold under real serving?" objection by measurement rather than argument:** re-running the V4 cells against a live `vllm serve` under N concurrent closed-loop agents (same `parallel_efficiency < 0.65` knee definition), the served knee is at or below the offline static knee on **26/26 distinct-traffic configurations** — the gap widening with context because distinct long prompts force prefill serialization the decode-only offline metric excludes (served efficiency collapses as ≈ 1/N from N=2 at ctx≥8192) — while under **shared-prefix traffic** (prefill amortized by the cache) the served knee lands on the offline *decode* knee within one concurrency-ladder step on **32/33 configurations**. The two regimes bracket the served knee: the decode-amortization mechanism transfers to serving once prefill is removed as a confound, and the offline knee is a *measured* conservative upper bound on the served operating point. **A third bandwidth point (H200, HBM3e 4.8 TB/s, 84-cell fp16/d64 grid, §12.3) was added and delivers a deliberate negative result:** the batch-saturation factor α does **not** track HBM bandwidth — the H100, not the higher-bandwidth H200, has the lowest α (matched fp16/d64 median 0.198 vs 0.298 at ctx≥8192; H200 α > H100 α in 11/12 cells), refuting the tempting two-point A100→H100 extrapolation and demonstrating (not just asserting) that α is an empirical per-GPU property. The predictive headline (knee ∝ 1/(C·k), ρ=0.84) is untouched; only the second-order α correction is shown to lack a bandwidth law. **Three follow-on cloud experiments then explain and stress-test that picture (§10.1, §12.4, §13.3).** (i) A **measured-bandwidth + MBU mechanism** (device-to-device probe: A100 1,763 / H100 3,020 / H200 4,240 GB/s, 86–90% of vendor spec) *explains* the α reversal: median MBU rises ×1.56 on the H100 (lowest α) but only ×1.27 on the H200 (highest α) — the H200's HBM3e bandwidth outruns the decode kernels (MBU saturates at ~0.74, never the ~1.0+ the A100/H100 reach), so more peak bandwidth buys *less* batch-amortization. (ii) An **fp8 KV-cache** sweep (H100, V1/FA3, 84 cells) delivers a second negative result: halving the KV-bytes-per-token did **not** push the knee out (median knee ratio 1.0, 0/6 later) against the model's ~2× prediction — fp8-KV is a capacity lever, not an amortization lever (FA3-vs-default backend confound flagged). (iii) A **128K-context** extension on Llama-3.1-8B (positive result) confirms the 1/(C·k) prediction 4× past the grid: the knee compresses to **batch 4** (architectural prediction ≈2–3), arriving at/before the 16.8 GB/sequence VRAM ceiling. This is the offline-amortization substrate reserved for the serving-stack-physics main-track paper — deliberately out of scope for the `v2_boundary` workshop paper per its claim ledger (which walls off 7B+/datacenter-hardware generalization). Exposure-clean (zero hits on the mandatory pre-push grep). Previous (2026-06-15): **TR164 V3 + TR167 — Phase 8 cross-backend capstone + JTPv2 predictive-validity** hand-promoted (PI directive). `Technical_Report_164_V3.md` (**913 lines / 28.5K words** full-depth, SS1–SS14 template-compliant) is the capstone of the TR164 line: the 5-model × 5-workload cross-backend serving-stack **boundary matrix on matched A100 80GB PCIe** (vLLM 0.10.2 vs SGLang 0.5.12.post1), **1,800 cells / 189,000 metric rows / ok_rate 1.0** across both backends. Headline: continuous batching produces a **gradual, workload-shaped** decline (both backends retain **0.47–0.62 parallel efficiency at N=32**) versus V1 pytorch_direct's catastrophic 0.056 at N=16 — a >10× upward shift of the breakdown boundary. The vLLM↔SGLang head-to-head uses paired **Wilcoxon + Holm-Bonferroni + percentile bootstrap CI** (seed 164, 10,000 resamples): SGLang Holm-significantly more efficient on short_decode / long_decode / repeated_prefix, vLLM on long_prefill_8k (Mistral-excluded), **balanced_2k a statistical tie** (CI straddles zero); all deltas small (1.5–7.6 pp), framed as a workload-conditional routing signal, not an engine ranking. The **matched SKU retires V2's bandwidth-adjustment sensitivity check** entirely (the load-bearing methodological advance over V2). One documented comparability caveat: Mistral × long_prefill_8k is tokenizer-confounded (mistral_common 8,148 vs HF 9,465 mean tokens, ratio 1.162) and **document-and-excluded**. All numbers trace to `research/tr164/v3_paper_stats.json`. `Technical_Report_167.md` (**1,186 lines / 42.1K words** full-depth) is the JTPv2 predictive-validity test of the JTP framework on the TR140 v3 / TR148 v2 GGUF-local corpus: 528 records / 24,000 judge labels / 10 headline cells. Verdict **Structural Degenerate Class** — the rlhf-only holdout is single-class (all judge-sensitive), so the pre-registered Req1/Req2 binary-discrimination battery (P1–P8) returns **INSUFFICIENT_DATA honestly** (ROC-AUC structurally undefined, DeLong/LRT collapse on the empty negative class) rather than laundering a synthetic 0.5; the **load-bearing secondary finding is P8 pool-robustness** (6 llama3.2 GGUF cells resurface judge-sensitive under the expanded cohort, mean kappa_min shift −0.1529). Substrate-honest depth: the length is earned by the full P1–P8 battery + degenerate-class mechanism + pool-robustness analysis, not by row count. Both are exposure-clean (zero hits on the mandatory pre-push grep); program total now **~1,337,000 primary + judge measurements** (`BANTERHEARTS_MEASUREMENT_COUNT.md` 2026-06-13 supplement: +189,000 TR164 V3 + 24,528 TR167). Previous (2026-06-08): **TR164 V2 + TR165 — Phase 8 Serving-Stack Mechanism Isolation Companion Reports** hand-promoted. `Technical_Report_164_V2.md` (**1,212 lines** full-depth, integrates the cross-run synthesis substrate from `research/tr164/cross_run_analyze.py` + `cross_run_analysis.json`) covers the V2 cross-backend matched-matrix expansion of V1's pytorch_direct findings — vLLM A100 PCIe + SGLang A100 SXM + TGI local RTX 4080 Laptop, 648 cells / 57,024 metric rows / ok_rate 1.0 across all backends. **The cross-run synthesis is the headline addition:** TGI vs V1 pytorch_direct matched-pair (n=120 cells) shows mean parallel-efficiency delta +0.2741 with **Cohen's d = 1.4361** and **Wilcoxon p < 6×10⁻²⁰**; the Mantel-Haenszel pooled OR for the binarized "efficiency ≥ 0.5" outcome, stratified by model × workload × phase, evaluates to **infinity from 22 of 24 strata** (TGI nearly universally passes the 0.5 threshold at N≥2 where V1 nearly universally fails). The H1 hypothesis (continuous-batching backends eliminate the V1 in-process N=2 breakdown) is empirically confirmed at the matched-pair statistical level. The vLLM-vs-SGLang head-to-head (n=48 cells) is hardware-confounded (PCIe 1.5 TB/s vs SXM 2.0 TB/s memory bandwidth), and the report reports both raw and memory-bandwidth-adjusted analyses: the raw delta of −0.017 (SGLang slightly higher, p=0.0006) flips under bandwidth adjustment to +0.246 (vLLM higher, p=1.6×10⁻⁹), confirming SXM hardware as the load-bearing contributor to SGLang's raw lead at the aggregate level. Per-workload Holm-Bonferroni stepdown isolates a real RadixAttention prefix-cache advantage on prefix-friendly workloads (SGLang wins balanced_2k +3.1pp at Holm-adj p=0.013, repeated_prefix +3.7pp at Holm-adj p=0.013) but null on long_decode and long_prefill_8k. The breakdown-boundary regression across all four backends at thresholds {0.9, 0.7, 0.5} shows V1 pytorch_direct's uniform N=2 breakdown is the most extreme pathology in the four-backend comparison; continuous-batching backends push the boundary by 1–4 N levels at moderate thresholds (TGI → N=8–16, vLLM → N=16–32, SGLang has half of combinations never breaking below 0.5 within N≤32). The companion `Technical_Report_165.md` (**1,294 lines** full-depth) covers the Python 3.14t free-threaded matched-pair test of the GIL-attribution hypothesis: 48 cells / 3,744 metric rows on RTX 4080 Laptop, single-delta design (only the Python interpreter build changes vs V1; same hardware, same models, same workloads, same backend). The pre-registered falsification verdict is **H2_partial**: H0 rejected (mean delta is meaningfully non-zero), H1 not supported (19/24 = 79.2% combinations clear the H1 threshold, 5/24 = 20.8% do not; 2/6 V1 deterministic-hang cell shapes resolved under nogil, 4 still hang), H2 supported. Mean N=2 parallel-efficiency uplift under nogil is **+0.1791 absolute (+17.91 pp)** with median +0.1715. The structural interpretation: GIL contention is A mechanism behind V1's N=2 breakdown but NOT the sole mechanism — the residual 21% of unimproved combinations and the 4 unresolved hangs implicate CUDA kernel serialization, memory-bandwidth contention, and dispatcher-layer overhead as co-contributors that survive GIL removal. Together TR164 V2 + TR165 form the Phase 8 two-axis matched-pair triangulation: V2 confirms backend-architecture-level breakdown elimination (continuous batching, clean), TR165 confirms Python-interpreter-level partial GIL attribution (nogil, partial). Both reports pass the mandatory blind-review exposure grep with zero hits. The cross-run synthesis artifact (`research/tr164/cross_run_analyze.py` + `cross_run_analysis.json` + `cross_run_summary.csv`) is the load-bearing deliverable for downstream consumption (bridge paper Phase 8 anchor, V3 wave-2 substrate when more cross-backend runs land). Previous: **2026-06-06** **TR163 v1 — RTSI-Gated Quantization Routing (first applied defense in the Phase 7 mitigation turn)** promoted by PI directive — `Technical_Report_163.md` (1,001 lines full-depth, 29 sections, Appendices A–D). Substrate: offline LOOCV reanalysis over the published TR142 v3 45-cell RTSI table (no new GPU sampling, ~16 KB of analysis JSON). Mechanism: config-level router consuming the published RTSI scorer at deployment-decision time; cost axis is the fraction of (model × quant) configurations routed to the safe baseline. **Pre-registered hypothesis verdicts: H0 rejected, H1 supported, H2 supported.** In-sample Pareto knee at threshold 0.5702 (3 / 45 cells routed = 6.67%, 38.67% of the aggregate 11.38pp refusal-rate-delta gap recovered, recovery-to-cost ratio 5.80); H1 supporting point at T=0.2754 (12 / 45 = 26.67% routed, 80.27% recovered); LOOCV high-band at T=0.40 (10 / 45 = 22.22% routed, 76.37% recovered, ROC-AUC=0.8445 vs the pre-registered H2 admissibility threshold of 0.75). The substrate inherits a **load-bearing circularity** (RTSI's four feature weights were calibrated against this same 45-cell matrix in TR142); the LOOCV pass removes cell-level circularity but does NOT remove family-level or quant-scheme-level leakage. **The honest weight of the v1 substrate rests on the LOOCV pass; the held-out-family generalization claim requires the TR163 v2 paper-grade campaign** (engineering specification complete in §SS15: ~75K primary records + ~225K triangulated judge labels under JTP + Mantel-Haenszel pooled OR + Holm-Bonferroni stepdown + TOST equivalence at ±3pp + held-out-family LOOCV, gated on Anthropic Fellowship + OpenAI Researcher Access Program umbrella). Cross-TR position: first applied use of the published RTSI screen (TR142) against the measured weight-quantization refusal gap (TR134); structural complement to TR155 (attention-sink eviction × refusal) and TR160 (refusal-direction geometry × quantization) in the Phase 7 mitigation set. Literature triangulation against SSD (arXiv 2508.17739) / Q-resafe (2506.20251) / Q-realign (2601.08089) at operating-resolution depth. Promotion to PublishReady is by **explicit PI directive ahead of the v2 expansion**; the report carries the structural caveats in-band (§SS9 circularity, §SS12 forbidden claims, §SS14 limitations, §SS15 v2 engineering spec) and is appropriate as a workshop-pilot artifact and as input substrate for the bridge paper's Layer 6 mitigation anchor when v2 lands. Previously (2026-06-05): **TR164 V1 — Serving-Stack Physics on Consumer GPU** shipped — `Technical_Report_164.md` (1,383 lines, full-depth with Appendices A-M) — closes the local_core leg of the SGLang + precision-controlled serving-stack expansion of the TR130-132 line. 346 real cells across 3 model tiers (llama3.2-1b, qwen2.5-1.5b, llama3.2-3b) × 2 phases (scaling, ttft) × 4 workloads (short_decode, balanced_2k, long_decode, repeated_prefix) × 5 concurrency tiers (N=1/2/4/8/16) × 3 reps, plus 14 skip-marker rows documenting six deterministic-hang cell shapes on llama3.2-3b at N=16. **Headline finding: parallel efficiency on the in-process pytorch_direct backend breaks down at N=2 uniformly across all 24 (model × workload × phase) combinations** — median efficiency 1.000 → 0.547 → 0.295 → 0.127 → 0.056 at N=1/2/4/8/16, P95 latency multiplier up to **1446.67× at N=16**, TTFT on llama3.2-1b N=16 balanced_2k at **188.4 seconds** for median request. **Six (phase × workload × N=16) cell shapes on llama3.2-3b proved deterministically pathological** — the dispatcher hung for ≥3.3 hours with GPU at 99% SM-occupancy but thermal/power readings consistent with GIL-starved idle (50-54°C, 23-49 W on a part rated 80-150 W sustained). Mechanism hypothesis: Python GIL serialization of in-process N-way concurrent inference dispatch, supported by (a) the thermal/power signature, (b) clean recovery via subprocess timeout + in-process fallback at lower N, (c) cross-model asymmetry where **qwen2.5-1.5b retains 0.92 mean speedup at N=16 while llama3.2-3b retains 0.03** on balanced_2k — consistent with model-architecture differences in GIL-release frequency. Substrate: 21,159 metric rows + 14 clean nsys-rep kernel captures (6.12 GB) + 3 orphan .qdstrm files (2.61 GB, NVIDIA Nsight Systems 2025.5.1 EventCollection bug). 7-generation dispatcher arc V1→V7 spanning 5d 13h 48m wall-clock (2026-05-31 12:04 → 2026-06-05 01:52). Methodological contributions: nsys-wrapped subprocess pattern (cell_worker.py), subprocess-timeout-and-fallback recovery, skip-row append protocol, capture_environment() Windows-WMI defensive guard. V2 cross-backend RunPod fan-out (vLLM, SGLang, Ollama, optionally TGI) + Python 3.14t nogil ablation are the mechanism-isolation experiments queued for the next compute window. Previous: **2026-05-28** **Conclusive Phase 6 synthesis trilogy** shipped — `Conclusive_Phase6.md` (1,453-line main synthesis), `Conclusive_Phase6_Extended_Appendices.md` (1,585-line per-TR appendix volume: Appendices A–G + cross-cutting X1–X4 + glossary), and `Conclusive_Phase6_Whitepaper.md` (408-line external decision whitepaper including the 5-layer protocol table, Qwen 2.5 1.5B worked example, and 8-objection reviewer FAQ). Consolidates seven executed reports — **TR144** (Speculative Decoding × Safety / TAIS), **TR145** (FP8 KV-cache single-config), **TR146** (mechanistic-probe falsification), **TR147** (Compile Reproducibility Index / CRI), **TR148** (Judge Triangulation Protocol / JTP), **TR149** (FP8 standardized batteries), **TR152** (FP8 serving-state factorial) — into a single **five-layer serving-state safety certification protocol** (Layer 1 JTP / Layer 2 RTSI + TAIS / Layer 3 CRI / Layer 4 scale validity / Layer 5 serving-state validity). Headline deliverable: a **serving-state-independent FP8 (E4M3) KV-cache certificate** for 1B–4B instruction-tuned models on vLLM v0.19.1 — harmful-refusal-neutral across batch ∈ {1, 8, 32}, prefix-caching {on, off}, and temperature ∈ {0.0, 0.7, 1.0} (TR152: **0 / 8,976 harmful-battery pairs discordant**; FP8-interaction spread 2.99 pp inside the ±3 pp band), with one located footnote — a sub-1 pp Qwen-2.5 over-refusal lean on the XSTest benign-edge battery (per-family Mantel–Haenszel OR 3.878 [2.386, 6.302]). The negative control (TR146) is load-bearing: four mechanistic probes all fail to distinguish safe from dangerous configs, so the entire arc stays behavioural — safety cannot be read off the weights. Claim ladder **C1–C10** (6 Supported / 2 Licensed / 2 Forbidden); the whole executed substrate ran at **$0 external API cost** under the umbrella gate on a local Ollama judge cohort. Cloud-gated envelope widenings (TR150 long-context, TR151 7B–72B scale, TR153 KV-method sweep) are gated on the 2026-10-24 GO/NO-GO trigger. The trilogy's most recent constituent report is TR152 v2 (Serving-State Safety Factorial — **canonical v2 local expansion**), shipped to `Technical_Report_152.md` (**1,341 lines full-depth narration with Appendices A-D**, supersedes the v1 pilot's 348 lines — matches TR148 / TR149 gold-standard template). SS14 tightened 2026-05-28 with v2's per-battery cross-judge κ recomputed directly from the sign-aware `per_judge_outcome` substrate (XSTest gemma↔llama κ = +0.7989 robust by JTP; harmful batteries degenerate-via-chance-correction-not-disagreement; pooled κ sanity-matches the analysis JSON exactly at 0.8310 / 0.0622 / 0.0962). Bridge-paper Layer 5: folds the FP8 KV-cache, batch, prefix-caching, speculative-decoding, and temperature axes into one `fp8_anchored_star` factorial (12 of 14 cells per model runnable; the 2 speculative-decoding cells fail uniformly on vLLM v0.19.1 argparse, not OOM — **v1's "cloud-gated by VRAM" attribution retracted**). **45,000 records, 20,754 matched FP16-vs-FP8 pairs across 5 models × 4 batteries (3 families)**, with XSTest uncapped to 450 / cell. Verdict: FP8 is **refusal-neutral on harmful prompts across the whole factorial** — HarmBench / JBB / StrongREJECT perfectly concordant (0 / 8,976 discordant pairs across 5 models × 6 contexts × 3 batteries), 0 / 120 cells Holm-significant, **117 / 120 TOST-equivalent at ±3 pp (97.5%, up from v1's 83.3%)**. The only footprint is a sub-1 pp over-refusal lean on the **Qwen family's XSTest** — **Mantel-Haenszel pooled OR 1.88 [1.32, 2.69] on 133 discordant pairs, sign-test p ≈ 0.0004**, refining v1's barely-clearing 2.69 [1.09, 6.62] on 23 pairs by 5.8 × the discordant base and tightening the CI by 4 ×. phi3-mini-4k and llama3.2-1b (both new in v2) pattern with the Llama-3.2-3b family-side: clean on XSTest, FP8 marginally improves their over-refusal behavior. The family signal localizes to Qwen 2.5 (both variants), amplified mildly by temperature on the 1.5 B variant. Run local-only under `--skip-openai-judge`, $0 external cost, 28.7 h end-to-end on RTX 4080 Laptop 12 GB. Previous: **2026-05-22** TR152 v1 (pilot, 348 lines — now retired in favor of v2 narration); **2026-05-14** TR149 (Standardized Safety Battery) → `Technical_Report_149.md` (1,303 lines) — FP8 KV-cache null replicates on the four standardized batteries (cross-battery MH pooled OR 0.8065 [0.38, 1.70], 0 / 12 Holm-significant, 12 / 12 TOST-equivalent); and **2026-05-13** TR148 v2 (Multi-Judge Reliability) → `Technical_Report_148.md` (1,556 lines) — triangulate verdict κ = 0.6917, dual-axis methodology finding. TR152 v2 judging completed 2026-05-28 00:06 ET — 135,000 judge-label rows now on disk, locking the program total at ~1,041,000 primary + judge measurements (see `BANTERHEARTS_MEASUREMENT_COUNT.md` 2026-05-28 supplement for verified row counts).
...
### Phase 7: Mitigation Turn (TR155, TR160, TR163) — v1 Promoted; v2 Expansions Queued

TR163 v1 promoted to PublishReady 2026-06-06 by PI directive — `Technical_Report_163.md` (1,001 lines, offline LOOCV over the TR142 v3 RTSI table). The substrate is workshop-pilot grade (45 cells, 17 positives, 28 negatives; LOOCV AUC=0.8445; H0 rejected, H1 supported at T=0.2754, H2 supported); the load-bearing circularity (RTSI calibrated on the same matrix) is disclosed in §SS9, and the paper-grade v2 expansion (~75K primary + ~225K judge, held-out-family, JTP+MH+Holm+TOST, external-acceptance-gated) is specified at engineering depth in §SS15. TR155 (936 + 2,808 judge, kvpress) and TR160 (built, not run) remain at MVP / pre-execution scale.

### Phase 8: Serving-Stack Mechanism Isolation (TR164 V1–V5, TR165) — Complete

TR164 V1 (pytorch_direct local_core, 5d 13h 48m dispatcher arc, 360 cells / 21,159 metric rows + 14 skip-marker rows), TR164 V2 cross-backend matched-matrix expansion (vLLM A100 PCIe + SGLang A100 SXM + TGI local RTX 4080 Laptop, 648 cells / 57,024 metric rows / ok_rate 1.0 across all backends), and TR165 Python 3.14t nogil ablation (48 cells / 3,744 metric rows / verdict H2_partial) form the Phase 8 two-axis matched-pair triangulation. The cross-run synthesis artifact at `research/tr164/cross_run_analysis.json` (produced by `cross_run_analyze.py`) joins V1 + V2 substrates and yields the paper-grade matched-pair stats: TGI vs V1 Cohen's d=1.44, Wilcoxon p<6×10⁻²⁰; vLLM vs SGLang bandwidth-adjusted +24.55pp p=1.6×10⁻⁹; Mantel-Haenszel pooled OR for efficiency≥0.5 = ∞ from 22/24 strata. The line extends through **V4** — the static-batch amortization surface (the η(B)=(1+r)/(1+Br), r=Ck/W bandwidth model + 672-cell static grid, 1,143 rows) — and **V5** evidence closure (predictor baselines + cross-family weight-axis matrix 708 rows + SGLang served-ceiling 12 rows), with TR165 contributing the 5,184-row same-session `PYTHON_GIL=1/0` control arms (counted 2026-06-24); the post-hoc SSP layer adds streaming/marginal-decode closure audits (252 rows), 70B confirmation (71 rows), live Modal policy-cap validation (8 rows), and the **TR170** kernel-reproducibility pilot (555 rows, separate TR). All five version reports (`Technical_Report_164.md` V1 + `_V2`/`_V3`/`_V4`/`_V5`) plus `Technical_Report_165.md` are promoted and carry a *Data Reconciliation & Cross-Version Lineage* section; the canonical run-dir → version → rows → finding map is `research/tr164/TR164_DATA_LINEAGE.md`. The TR164 line underwrites the Serving Stack Physics submission (`papers/serving_stack_physics/tmlr_whole_draft/`, submitted to TMLR 2026-06-24); the bridge-paper Phase 8 anchor at `papers/serving_state_safety_certification/UPGRADE_PLAN.md` consumes the V2+TR165 substrate.
...
**Total Reports:** 49+ completed TR versions including the Phase 6 serving-state safety arc (TR144, TR145, TR148, TR149, TR152) + TR147 four-subversion benchmarking-integrity track + 3 v2 expansions + 3 v3 AWQ/GPTQ expansions + TR138 Study D Addendum (ICML camera-ready supplement) + 21 conclusive/whitepaper documents (7 phase groups × report / appendices / whitepaper) + 7 historical/superseded + 3 legacy + model benchmarks
**Total Measurements:** **~1,348,000 primary + judge measurements on disk across 49+ TRs** (2026-06-24 supplement = +11,781 over the 2026-06-13 baseline: 6,042 TR164 SSP substrate + 5,184 TR165 GIL-control arms + 555 TR170 pilot); see `BANTERHEARTS_MEASUREMENT_COUNT.md` (2026-06-24 supplement) for canonical reconciliation with verified row counts. Prior baselines: ~1,343,000 (2026-06-24 pre-GIL-fix), ~1,337,000 (2026-06-13 post-TR164 V3 + TR167), ~1,123,000 (2026-06-08 post-TR165), ~1,041,000 (2026-05-28 post-TR152 v2).
