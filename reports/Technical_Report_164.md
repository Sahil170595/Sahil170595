...
**The Banterhearts measurement-program-wide bridge.** TR164's V1+V2 substrate fits in the larger Banterhearts research program — approximately 1,348,000 primary + judge measurements across 49+ unique TR numbers as of the 2026-06-24 supplement (this supersedes the prior ~1,041,000 / 45+ TR figure carried in earlier drafts, which was a 2026-05-28 snapshot stale by ~302K measurements and ~4 TRs). The V1 leg contributes approximately 21,159 measurements at the request level, with the now-executed V2 leg contributing 57,024 additional measurements across the cross-backend × cross-model × cross-workload sweep (TGI 26,784 + vLLM 15,120 + SGLang 15,120). The bridge paper synthesizes this larger substrate; TR164 is one leg of approximately ten that the bridge paper integrates.
...
> **Scope banner.** This DRAFT documents ONLY the TR164 V1 closed-loop boundary — the in-process `pytorch_direct` local_core leg. The V2–V5 cross-backend grids and all post-hoc closure/static-model/served/policy work live in separate artifacts (the V2/V3 DRAFT reports, the eight `research/tr164/methodology/TR_*.md` files, and the SSP submission). Nothing below changes a single V1 number; it only places V1 in the arc and corrects one stale program-total citation (Appendix M). The canonical map is `research/tr164/TR164_DATA_LINEAGE.md`.

**(a) This version's exact provenance.** V1 is run-dir `research/tr164/results/20260531_120428_552237`, **21,159 primary metric rows** in `metrics.csv` (per-request throughput/latency), across **346 real cells + 14 zeroed skip-marker rows = 360 entries**, 100% status='ok' after skip-row reconciliation. This row count matches the `BANTERHEARTS_MEASUREMENT_COUNT.md` 2026-06-05 supplement to the row (+21,159, +1 TR; verified on disk 2026-06-24 as 21,160 lines − 1 header). The headline numbers in this draft — P95 wall-latency up to **1446.67× at N=16**, **14 clean `.nsys-rep` captures + 3 orphan `.qdstrm`** (the nsys 2025.5.1 `EventCollection.cpp:1054` importer bug), and the six deterministic-hang cell shapes on llama3.2-3b — are all accurate as stated. **Hardware.** V1 pytorch_direct ran on a single RTX 4080 Laptop 12GB (local, in-process) — run `20260531_120428_552237`.

**(b) V1's place in the TR164 arc.** V1 *opens* the program. It establishes the metric the whole arc rests on — `PE(N) = agg_decode_tps(N) / (N · agg_decode_tps(1))` — and discovers the uniform parallel-efficiency breakdown at the very first concurrency step (N=2) across all 24 model×workload×phase combinations, framing the GIL as an *emergent* (not pre-registered) attribution. Nothing came before V1 in this line beyond the predecessor TR130–132 four-backend baseline. What came after: V2 (does a real server backend move the boundary — yes), V3 (matched-hardware vLLM/SGLang head-to-head), V4 (a predictive static model), V5 (reviewer-facing evidence closure), and the TR170 kernel-reproducibility pilot.

**(c) Work done AFTER V1 that extends or re-interprets it** (each with its real run-dir, scoped to what touches V1):
- **GIL attribution falsified to H2_partial — TR165 no-GIL ablation** (`research/tr165/results/20260607_174748_273070`, **3,744 rows**). Under a free-threaded CPython 3.14.5 build, 19/24 N=2 combos clear the H1 threshold and 2/6 V1 deterministic-hang shapes resolve: the GIL is *a contributing, not the sole* mechanism behind V1's N=2 breakdown. **This V1 draft predates TR165 and does not reference it**; the GIL-attribution language in §3/§5 above should be read as the V1-era emergent hypothesis, now superseded by the H2_partial verdict.
- **V2 cross-backend** (TGI local_core `results/20260605_192757_415750`, 26,784 rows; vLLM cloud_core `results/20260605_210337_450607`, 15,120; SGLang cloud_core `results/20260605_212557_266597`, 15,120). TGI eliminates V1's uniform N=2 collapse (N=2 efficiency stays >0.65; the six V1 hang cells run clean) — the direct re-interpretation of V1's headline.
- **V3 matched-hardware head-to-head** (vLLM `results/20260612_204254_036590`, SGLang `results/20260612_212816_266127`, 94,500 rows each on A100 80GB PCIe). Workload-conditional sign flip, no universal engine winner; row-level auditing caught a 3,780-row HTTP-400 mass fixed by the **Matched SGLang refill** (`results/20260613_055519_963364`, 3,780 rows).
- **V4 static `η(B)=(1+r)/(1+Br)`, r=Ck/W model** (`results/modal_amortization`, 1,143 rows). Converts V1's *descriptive* breakdown into a *predictive* knee from architecture alone (zero-param Ck/W orders finite knees at ρ≈0.84).
- **Closure audits** — streaming true-decode (`vllm_decode_timed_closure`, 252 rows) + marginal-decode (`mlsys_closure`, derived). Ck/W rank survives isolating true steady-state decode (magnitude tightens 2.022→1.322).
- **Cross-family weight-axis matrix** (`weight_axis_decode`, 708 rows). Isolates the W lever across Qwen/Mistral/Gemma3; pooled cross-family ρ=0.8142, Gemma3 4B→12B inversion blocks any "larger always later" universal.
- **70B confirmation** (`results/modal_amortization_70b`, 31 rows + `results/modal_amortization_70b_confirm`, 40 rows). 70B knee ~1.8× later than 8B supports W-ordering (TP=2 aggregate-bandwidth confound flagged).
- **Served-ceiling validation** (`results/modal_closed_loop`, 68 rows + `v5/results/sglang_served`, 12 rows). Served ≤ static on 26/26 distinct-traffic + 8/8 SGLang configs.
- **Live Modal policy-cap validation** (`policy_replay/modal_policy_validation`, 8 rows). Operationalizes the knee table into a deployable batch-cap policy.
- **TR170 kernel-reproducibility pilot** (`research/tr170/results`, **555 rows** — noise-floor 3 + tritonbench sweep 184×3 across Triton 3.3.1/3.4.0/3.6.0). Drops one level below TR164's serving physics; pilot-grade, not paper-grade.

**(d) Methodology cross-references.** V1's method is documented in `research/tr164/methodology/TR_closed_loop_boundary.md` (the §4 closed-loop boundary). Indexed via `TR164_METHODOLOGY_INDEX.md`. Downstream methodology docs relevant to the post-V1 re-interpretation: `TR_nogil_ablation.md` (the H2_partial falsification of V1's GIL claim), `TR_static_ckw_model.md` / `TR_roofline_mechanism.md` / `TR_extension_probes.md` (V4), `TR_weight_axis_matrix.md` (V5), `TR_served_validation.md` (served/policy), and `TR_mlsys_closure.md` (closure). The live-Modal policy validation and TR170 have no dedicated TR164 `TR_*.md` (marked NOT-verified in the lineage map's methodology column).

**POST-HOC: SSP extensions.** V1's 21,159-row closed-loop boundary became **one leg of the SSP closed-loop figure**, audited at **274,707 metric rows = V1 (21,159) + V2 TGI/vLLM/SGLang (26,784 + 15,120 + 15,120) + V3 vLLM/SGLang (94,500 + 94,500) + Matched-SGLang refill (3,780) + TR165 nogil (3,744)**. Of these, only the 3,780 refill was newly counted within the figure — the other ~268K were already counted under earlier supplements, so the SSP headline must NOT be summed onto the program total (double-count guard). The V4 static-model, weight-axis (708 rows), closure (252 rows), 70B (31+40 rows), served-ceiling (12 rows), policy-replay (8 rows), and TR170 (555 rows) substrates **extend beyond this report**; the SSP post-hoc increment is 6,042 primary rows + TR170 555 = 6,597 (the full 2026-06-24 supplement is +11,781, also folding in the 5,184-row TR165 GIL-control arms surfaced by the cross-version reconciliation), taking the program headline to ~1,348,000. To trace V1 into the closed-loop accounting and avoid double-counting, cross-reference run-dir `20260531_120428_552237` against the SSP freeze commits **`d58bfc44`** (submission snapshot + TR170 pilot) and **`5b7a5736`** (froze submitted package) on branch `codex/ssp-tmlr-compress`. Generic venue note: the downstream paper is a full-depth systems-venue / TMLR submission; no acceptance-conditional language attaches to V1.
