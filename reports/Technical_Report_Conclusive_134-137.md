...
## Synthesis of quantization-induced alignment erosion, concurrency invariance, backend-driven template divergence, and cross-axis safety taxonomy for local-first LLM deployment
...
|----------|-------|---------------|--------|
| C1 | Quantization degrades safety alignment | TR134: 4 models x 7 quant levels x 6 tasks = 24,778 samples. Llama 1B loses 35.2pp (d = 1.93) at Q2_K; Mistral 7B slope = +0.041. But Llama 3B gains 6.0pp at Q2_K. | **Demonstrated** for specific models; **not universal** (I-squared = 99.9%) |
| C2 | Concurrency does not degrade safety | TR135: 3 models x 4 N-levels x 6 tasks = 39,060 samples. Max delta 0.4pp. All 12 jailbreak slopes = 0.000. I-squared = 0.0% (all models agree). | **Demonstrated** |
| C3 | Backend choice affects safety | TR136: 3 models x 4 backends x 6 tasks = 10,416 samples. Ollama-to-vLLM costs 6-25pp safety (model-dependent). 0/18 TOST equivalence tests pass at +/-3pp. | **Demonstrated** |
...
| C5 | Jailbreak susceptibility increases at lower quantization | TR134: All 4 technique slopes negative (-0.021 to -0.036 per BPW). Prefix injection most effective. | **Demonstrated** |
| C6 | Jailbreak susceptibility is invariant to concurrency | TR135: All 12 model-technique slopes = 0.000. | **Demonstrated** |
...
| C8 | No backend pair is safety-equivalent at +/-3pp | TR136: 0/18 TOST tests pass. Even vLLM vs TGI (d < 0.03) fails TOST. | **Demonstrated** |
| C9 | Nationality is the most vulnerable bias category | TR134: Avg slope = -0.010/BPW across 4 models. Race/Ethnicity = +0.015 (least vulnerable). | **Demonstrated** (directionally; per-model variance is high) |
...
Open-weight LLMs that run on consumer hardware are already widely available. Anyone with a laptop GPU can download a quantized model, serve it locally, and deploy it for applications ranging from personal assistants to customer-facing chatbots. The tooling for local deployment (Ollama, llama.cpp, vLLM) is mature and accessible. What is missing is safety evaluation infrastructure — a systematic way to know whether the optimizations required to fit a model on a consumer GPU (quantization, batching, backend choice) have degraded its safety alignment. Without this infrastructure, consumer LLM deployments risk generating harmful content, failing to refuse dangerous requests, or exhibiting demographic biases that were controlled at full precision but eroded at deployment precision.
...
Evidence base: TR135 (39,060 samples, strong null finding).
