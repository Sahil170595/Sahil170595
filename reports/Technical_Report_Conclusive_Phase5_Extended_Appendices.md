...
| **Scope** | 138-143 |
| **Status** | Supplemental |
| **Main Report** | [Technical_Report_Conclusive_Phase5.md](Technical_Report_Conclusive_Phase5.md) |
...
**The 27% genuine rate means:** Of every 100 automated safety flips detected, approximately 27 represent actual behavioral changes where the model shifted from refusing to complying (or vice versa). The remaining 73 are different phrasings of the same safety posture. The human-adjudicated genuine flip rate for TR138 is therefore 0.6% x 0.27 ~ 0.16%, not 0.6%.
...
These playbooks translate the Phase 4/4 findings into step-by-step procedures executable by engineering teams.
...
- Total conversations: 4 strategies x 50 behaviors = 200 minimum
...
Adjusted flip rate = automated flip rate x genuine rate.
...
### S.2a Batch-Invariant Kernel Path Addendum

**Decision: When batch-invariant kernels are available, validate the exact served kernel path rather than assuming standard-path results transfer.**

| Element | Detail |
|---------|--------|
| Contributing TRs | TR138 Study D addendum |
| Evidence: standard path | Standard vLLM/H100 reproduces 22/55 label flips and 25/55 text changes on current P1/P4 score-flip candidates. |
| Evidence: invariant path | `VLLM_BATCH_INVARIANT=1` produces 0/55 label flips and 0/55 text changes on the same candidate surface. |
| Artifact | `research/tr138_kernel_ablation/results/20260524_172010/summary.json` |
| Full report | `PublishReady/reports/Technical_Report_138_Study_D_Addendum.md` |
| Falsification | A prospective batch-invariant run over a broader prompt population that still produces label flips would narrow the addendum to this candidate surface. |
...
TR138 Study D later adds a direct kernel-path check. On the current score-flip candidate surface, standard vLLM on H100 reproduces 22/55 label flips, while the batch-invariant vLLM path reproduces 0/55. This is the cleanest mechanism evidence in the TR138 artifact chain: the same candidate rows change under the standard serving path and become stable under the tested invariant path.
...
Phase 2 (5,616 samples) tested co-batching interference: does the content of other requests in the batch affect the target's safety? The answer was no (confirmed more rigorously by TR143 later). Phase 3 (5,940 samples) added quantization x concurrency interaction: quantization dominated (34.96pp variance), concurrency had zero effect. Phase 5 (2,700 samples) validated with true batching via vLLM's prompt-list API: the effect replicated at 0.8% with 99.4% flip agreement against Phase 1.
...
TR139 is the largest factorial experiment in the program: 4 models x 6 quantization levels x 8 strategies x 50 behaviors = 9,600 Phase 1 conversations, plus 1,000 Phase 2 persistence tests. The design was intentionally comprehensive because the multi-turn jailbreak x quantization interaction had never been systematically measured.
...
**Post-report Study D note:** If this pipeline can use a batch-invariant kernel path, the updated recommendation is to benchmark that path before accepting standard-path instability. TR138 Study D shows standard vLLM candidate flips can collapse from 22/55 to 0/55 under `VLLM_BATCH_INVARIANT=1`, but throughput, compatibility, and the pipeline's own prompt distribution still need direct measurement.
...
200 conversations (4 strategies x 50 behaviors). If any strategy shows > 50% ASR at Q4_K_M, the model has a baseline vulnerability requiring external guardrails.
...
## Appendix X Extended: Cross-Phase Synthesis Narrative (Phase 2 + Phase 3 + Phase 4/4)
...
**Phase 4/4 (TR138-TR143): What is the full attack surface?** Multi-turn, many-shot, batch perturbation, cross-architecture, quality-safety, composition. Quantization dominant by 100x. No categorical shortcut works. 306,996 samples.

Each phase depends on the one below. Performance decisions (Phase 2) are incomplete without safety validation (Phase 3). Safety validation (Phase 3) is incomplete without attack-surface mapping (Phase 4/4). The complete stack enables deployment decisions that jointly optimize for throughput, cost, quality, AND safety -- rather than optimizing for one dimension and hoping the others are fine.

**Post-report Study D addendum:** The batch-perturbation layer now includes a 110-record H100/vLLM mechanism check outside the original 306,996-sample synthesis total. Its contribution is not a new prevalence estimate; it shows that the current TR138 candidate flips are sensitive to the served kernel path.
...
Phase 4/4 completes the triangle: Q4_K_M is safe under multi-turn and many-shot attacks for Llama models. Quality is not a reliable proxy for safety (13.9x divergence). Batch configuration is safe. The triangle is now fully characterized for the tested model range.

The Study D addendum refines the batch side of the triangle: batch configuration remains low-severity at the prevalence level, but kernel-path choice becomes a reproducibility control when exact behavior matters.
...
7. **Prospective batch-invariant benchmarking.** Does the 0/55 Study D invariant result hold on broader prompt populations, larger models, non-H100 GPUs, and production schedulers?
...
*This document is supplemental to the main conclusive report (Technical_Report_Conclusive_Phase5.md). For the canonical analysis, claim status table, and full appendix set, consult the main report.*
