...
| Hypothesis | Result | Delta Efficiency | Status |
|------------|--------|--------------|--------|
| H_1: Tokio LocalSet reduces scheduler overhead | WARNING INCONCLUSIVE | +0.1pp | Within noise |
| H2: Async-std provides lower-latency coordination | FAIL REJECTED | -45pp | Critical failure |
| H3: Smol runtime matches Tokio with less overhead | PASS CONFIRMED | 0pp | Identical performance |
| H4: 1KB HTTP buffering improves TTFT | WARNING MARGINAL | +1.1pp | Small but peak-achieving |
...
- **Scenarios:** 6 configurations (baseline, hetero, 4x homo variants)
- **Replication:** 5 runs per config x 5 runtimes = **150 total benchmarks**
...
**Primary:**
- **Parallel Efficiency (%):** `(Sequential_Time / (Concurrent_Time x num_agents)) x 100`
- **Concurrency Speedup (x):** `Sequential_Time / Concurrent_Time`
...
| Runtime | Peak Eff (%) | Mean Eff (%) | StdDev (pp) | Peak Speedup (x) | vs TR114 (pp) | vs TR110 (pp) |
|---------|--------------|--------------|-------------|------------------|---------------|---------------|
...
| Report | Architecture | Peak Eff (%) | Delta from Prior | Key Improvement |
|--------|--------------|--------------|--------------|-----------------|
...
**Sample Size:** 5 runs per config x 6 configs x 5 runtimes = 150 benchmarks
