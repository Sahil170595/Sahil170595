...
**Observations.** The architectural prediction sits systematically *below* the observed knee (points above the y = x line in Figure 3) by a roughly constant factor — the batch-saturation factor α = r_fit / r_arch, median **0.52 on the A100 and 0.33 on the H100**. A first-principles roofline check (§9.3, §11.3) shows this is **not** compute overlap — decode is memory-bound throughout (compute is 4.6% of step time) — but the linear-byte model saturating at high batch: above the VRAM concurrency ceiling `n_max = VRAM/(C·k)` the KV cannot be held B-way concurrent, so effective traffic is sublinear. The same KV footprint that sets `r` sets `n_max`, and the observed knee tracks `n_max` at ρ = 0.83 (≈ 0.27 × n_max). Either way the operational payoff stands: from three numbers in a model's config and the GPU's HBM/VRAM, one can predict to within a factor of ~2 the batch at which continuous batching stops paying at a given context.

> The amortization knee is predictable. A one-parameter bandwidth model fits the curves at R² 0.93 and its zero-parameter architectural form predicts the knee at ρ = 0.84; the residual is a batch-saturation factor whose source is the VRAM concurrency ceiling (the knee independently tracks `n_max = VRAM/(C·k)` at ρ = 0.83), not compute overlap. This turns "long context is slower" into a usable engineering prediction.
...
V1–V3 described *where* the boundary sits. V4's ambition is to *predict* it. The closed-loop ladder cannot do this cleanly because it folds the amortization physics together with scheduler decisions and arrival noise; the static-batch grid strips those away, leaving a quantity — `η(B)` — that a first-principles bandwidth argument predicts in closed form. The test of the report is whether that argument, fed only verified architecture and hardware specs, predicts the measured knee. It does, to within the batch-saturation factor (whose source §11.3 identifies as the VRAM concurrency ceiling), which is the report's main contribution: a serving engineer can estimate the amortization knee for an untested (model, GPU, context) from the model card and the datasheet.
...
The model `η(B) = (1+r)/(1+Br)` with `r = C·k/W` is derived in §9 from a bandwidth-bound decode step. Its parameters are computed from verified architecture (Appendix D): weight bytes `W = params × 2` (fp16) or `× 1` (fp8, approximate); KV-bytes-per-token `k = 2 × layers × KV-heads × head_dim × 2` (the leading 2 for K and V, the trailing 2 for fp16). For each curve we fit `r` by least squares (the architectural value is *not* used in the fit), report R², compare the fitted `r` to the architectural `r_arch`, and define the batch-saturation factor `α = r_fit / r_arch` (§11.3 shows it is a VRAM-capacity effect, not compute overlap). The architectural knee `B* = (0.35 + r_arch)/(0.65·r_arch)` is the zero-parameter prediction validated against the observed knee; the VRAM concurrency ceiling `n_max = (0.9·VRAM − W − overhead)/(C·k)` is an independent, capacity-based predictor of the same knee (§11.3).
...
The derivation assumes the per-step memory traffic grows *linearly* in batch as `W + B·C·k`. Observed η is consistently *gentler* than this predicts — the real curve behaves as if `r` were `α · r_arch` with `α < 1` (median 0.52 on A100, 0.33 on H100, §11.3) — so the pure-architectural prediction (`α = 1`) is a conservative floor on the knee. The mechanism is **not** compute overlap. A first-principles roofline check on all 672 cells (`derive_alpha.py`) shows decode is memory-bound essentially everywhere: the compute-to-memory time ratio is a median of **0.046** (4.6%), exceeding 0.5 in only 6 of 576 cells, and reaching even 0.36 only at the short-context, high-batch corner (ctx512, batch 64) where the weight matmul is briefly a non-trivial fraction. There is no compute headroom to "hide behind the reads" in the long-context regime that sets the knee — decode there is 20× memory-bound. Instead, `α < 1` is the signature of the **linear-byte assumption itself breaking down at high batch**: the implied memory-bandwidth utilization computed from the model, `MBU(B) = t(B)·(W+B·C·k)/BW_peak`, *exceeds 1.0* at large batch (up to 1.2 on the A100 and 1.95 on the H100 at batch 64) — physically impossible, so the engine is moving *less* than `B × C·k` of KV from HBM per step. The dominant cause is concrete and capacity-grounded: at the long-context, high-batch corners the KV cache that `B·C·k` would require **exceeds GPU memory** (e.g. Llama at batch 64 × 32,000 tokens implies ≈ 268 GB of KV against 80 GB of HBM), so vLLM cannot run `B`-way concurrent and caps the running set at the largest batch whose KV fits VRAM, processing the rest in waves. Effective KV traffic is therefore *sublinear* in submitted batch, which is exactly what makes the curve gentler. **Observations.** This reframes α from a fitted "compute-overlap" constant into a symptom of a capacity boundary — and §11.3 shows the knee tracks that boundary (the VRAM concurrency ceiling) at the same ρ ≈ 0.83 the bandwidth ratio predicts it, because both are functions of the same KV footprint `C·k`.

> The bandwidth model is derived, not assumed: a bandwidth-bound decode step yields `η(B) = (1+r)/(1+Br)` with `r = C·k/W`, predicting an inverse-context knee and a model ordering set by KV-bytes-per-token. Decode is memory-bound throughout (compute is 4.6% of step time), so the gentling `α < 1` is *not* compute overlap — it is the linear-byte assumption saturating at high batch, dominated by the regime where the KV cache exceeds VRAM and the scheduler wave-batches. The pure form is a conservative floor; α is the correction for that capacity boundary.
...
Computing `r_arch = C·k/W` from verified architecture (no fitting) and the closed-form knee `B* = (0.35 + r_arch)/(0.65·r_arch)`, the predicted knee correlates with the observed interpolated knee at **Spearman ρ = 0.84** (Pearson on log₂ 0.79, n = 65 breaking curves; `L3_roofline.arch_knee_vs_obs_knee_spearman`, Figure 3). The prediction holds across two orders of magnitude: for `qwen2.5-7b | H100 | d64 | fp16` the architectural knee runs 281 → 71 → 19 → 6 across contexts 512 → 2,048 → 8,192 → 32,000, against observed never → never → 41 → 11 — same ordering, same scale, no fit. **Observations.** The points in Figure 3 sit systematically *above* the y = x line: the observed knee is later than the pure-architectural prediction, consistently, because effective KV traffic saturates at high batch (α < 1) — the VRAM-capacity effect §11.3 identifies, not compute overlap. The offset is the signal, not noise — it is the batch-saturation factor quantified next. That a knee computed from three config numbers and a context length predicts the measured breakdown at ρ = 0.84 is the report's central claim.

### 11.3 The batch-saturation factor α, the VRAM ceiling, and where the model breaks

The ratio `α = r_fit / r_arch` measures how much gentler the real curve is than the *linear-byte* pure model predicts; its median is **0.52 on the A100 and 0.33 on the H100** (`L3_roofline.alpha_compute_overlap_by_gpu`). The first-principles derivation (§9.3, `derive_alpha.py`) settles what α is — and what it is not. **It is not compute overlap:** decode is memory-bound throughout (compute/memory time median 0.046; > 0.2 in only 46 of 576 cells, all at short-context/high-batch), so there is no compute to hide the KV reads behind in the long-context regime that sets the knee. **It is the linear-byte assumption saturating at high batch:** the model-implied `MBU(B) = t(B)·(W+B·C·k)/BW_peak` exceeds the physical ceiling of 1.0 at large batch (to 1.2 on A100, 1.95 on H100 at batch 64), so effective KV traffic is sublinear in submitted batch. The dominant, testable cause is the **VRAM concurrency ceiling**: vLLM holds at most `n_max = (0.9·VRAM − W − overhead)/(C·k)` sequences of length C, so at batches above `n_max` it cannot run B-way concurrent and wave-schedules. The decisive check: the **observed knee tracks `n_max` at Spearman ρ = 0.83** (n = 65 breaking curves, `Q4_vram_ceiling`) — the same strength as the bandwidth ratio's ρ = 0.84 — with the knee landing at a median **0.27 × n_max**. Both predictors work because both are functions of the one KV footprint `C·k`: `r = C·k/W` (the bandwidth view) and `n_max = VRAM/(C·k)` (the capacity view). The H100's lower α is therefore not "more compute overlap" but a slightly larger effective-traffic relaxation; the cleaner reading is that **its higher HBM bandwidth holds η up (§12) while the knee's *location* is governed by the shared KV-footprint, capacity-bounded near VRAM.** The pure model fails outright in exactly one place — the four `H100 | d512 | ctx32000` curves return R² < 0 because they are **non-monotone** (η dips then partly recovers toward batch 64), the signature of the wave-scheduling regime re-organizing the batch at the most VRAM-stressed corner. **Observations.** The honest status of the model: an excellent *empirical* knee-predictor (ρ ≈ 0.84) whose closed form is exact only where the KV fits VRAM, with α the correction for the capacity boundary above it — not a fitted compute-overlap constant. This is a stronger result than the original framing claimed, and a correction of it: the mechanism is KV-footprint-and-capacity, dual-predicted, not compute.

> α is the batch-saturation correction, not compute overlap (decode is 4.6%-compute, memory-bound throughout). Its source is concrete: the knee tracks the VRAM concurrency ceiling `n_max = VRAM/(C·k)` at ρ = 0.83 — the same strength the bandwidth ratio `r = C·k/W` predicts it (ρ = 0.84) — because both are functions of the KV footprint. The knee sits at ~0.27 × n_max: continuous batching stops paying at roughly a quarter of the batch that would fill VRAM. The model is an exact predictor where KV fits VRAM and a conservative floor above it.
...
> The knee compresses as context^(−0.64), a measured rate between the pure-bandwidth prediction (−1) and context-independence (0); the gap from −1 is the batch-saturation relaxation (the VRAM-ceiling effect of §11.3), the same α in a third guise.
...
The substrate bounds the claims. **Three models, one scale band** (7–8B; not a size sweep — V3 carries the size gradient; nothing about sub-7B or 13B+). **Two GPUs, one engine version** (A100-80GB, H100; vLLM 0.10.2 V1; the bandwidth axis is a two-point contrast, and the α factor is GPU-pair-specific, though the mechanism is engine-independent). **fp8 weight, not fp8 KV** (the KV cache is fp16 in every cell — the cause of the fp8 penalty's sign and the motivation for the fp8-KV follow-up; no fp8-KV claim is made). **Static batch, not closed-loop** (every cell is one fixed equal-length batch; the amortization ceiling, not a served latency; no p95 or throughput-under-traffic figure — §24). **Equal-length prompts** (real batches are ragged; the equal-length static batch is the best case, so the measured knee is an upper bound on a real scheduler's). **Greedy fixed-length decode** (no sampling or early stopping). **The 32,000-token ceiling** (shared across models with different position-embedding limits; the 131,072-token Llama regime is untested). **The roofline model's scope** (the pure form assumes linear KV byte growth; it fits 85% of curves but breaks on the non-monotone H100/d512/ctx32000 corner where the scheduler wave-batches above the VRAM ceiling, and while α's *mechanism* is identified (the VRAM concurrency ceiling, §11.3) its multiplicative *value* is an empirical per-GPU summary, not a closed form). **The VRAM-ceiling model's assumptions** (n_max uses an assumed 0.9 memory-utilization and a ~3 GB overhead; the qualitative knee≈n_max link is robust, the 0.27 ratio is substrate-specific). **Nominal peak HBM** (MBU uses vendor-spec peak bandwidth, not a measured bandwidth ceiling). **Observations.** None of these threatens the headline, which is a predictive mechanism and a ranking on a defined substrate, not a universal constant. The limitations bound *generalization*, enumerated next.
...
7. **An exact closed-form α** — α's *mechanism* is identified (the VRAM concurrency ceiling, §11.3), but its multiplicative value is an empirical per-GPU summary, not a closed form; the bandwidth model is licensed as predictive-up-to-α, exact only where the KV fits VRAM.
8. **Universal verbs** — "always", "proves", "eliminates", "guarantees" are banned; the licensed reading is "on the 672 cells measured, the knee is governed by the KV footprint C·k, predicted from architecture at ρ = 0.84 (bandwidth) and ρ = 0.83 (VRAM ceiling), up to a GPU-specific batch-saturation factor."
9. **A Llama-versus-Mistral robustness ordering** — a statistical tie (Holm p = 0.13); reported as a tie (§16).

> The result is strong inside its box and silent outside it. The box is three 7–8B models, two GPUs, one engine, static-batch offline throughput, and a bandwidth model predictive up to an empirical batch-saturation factor whose mechanism is the VRAM concurrency ceiling; every headline claim is bounded to that box.
...
- Analyzers — `research/tr164/analyze_amortization.py`, `research/tr164/analyze_amortization_full.py`, `research/tr164/derive_alpha.py` (the first-principles mechanism derivation → `alpha_derivation.json`: compute-boundedness, MBU>1 at high batch, and the VRAM-ceiling knee predictor of §11.3).
...
Representative rows from `L3_roofline.per_curve` (fitted `r`, architectural `r`, batch-saturation α, fit R², architectural-predicted knee, observed interpolated knee). Full 96-curve table in the JSON; the VRAM-ceiling derivation that explains α is in `alpha_derivation.json` (§11.3).
...
**Observations.** The architectural knee tracks the observed knee in order and scale across the context range (281 → 19 → 6 vs — → 41 → 11), under-predicting by the batch-saturation factor (α ≈ 0.38 here, near the H100 median 0.33; a VRAM-capacity effect, not compute overlap — §11.3); the negative-R² row is one of the four non-monotone H100/d512/ctx32000 tails where the monotone form cannot fit (§11.3).
...
| `alphaA100` / `alphaH100` | 0.524 / 0.330 | `f.L3_roofline.alpha_compute_overlap_by_gpu` (JSON key name retained) | batch-saturation factor r_fit/r_arch — a VRAM-capacity effect, NOT compute overlap (§9.3, §11.3) |
| `kneeVsVramSpearman` | 0.83 | `derive_alpha.py` → `alpha_derivation.json` `Q4_vram_ceiling` | knee vs VRAM ceiling n_max=(0.9·VRAM−W)/(C·k), n=65 |
| `kneeOverNmaxRatio` | 0.27 | `…Q4_vram_ceiling.median_knee_over_nmax_ratio` | knee sits at ~¼ of the VRAM concurrency ceiling |
...
| 3 | `fig_arch_knee_pred.png` | `make_amortization_figures_full.py` | "Zero-free-parameter architectural knee B*=(0.35+r)/(0.65r), r=C·k/W, vs observed interpolated knee (log-log, n=65 breaking curves, Spearman ρ=0.84); points sit above y=x by the batch-saturation factor α (the VRAM-ceiling correction, §11.3)." Supports §11.2 (the predictive validation — the paper's headline figure). |
...
8. *The breakdown knee is governed by the KV footprint C·k and predicted two complementary ways at equal strength — the bandwidth ratio r=C·k/W (ρ=0.84) and the VRAM concurrency ceiling n_max=VRAM/(C·k) (ρ=0.83) — with the knee landing at ~0.27×n_max: continuous batching stops paying at roughly a quarter of the batch that would fill VRAM.*

**Tier 2 — Licensed with caveat (state only with the bracketed qualifier):**
- The knee prediction is *predictive up to a GPU-specific batch-saturation factor α* (0.52 A100, 0.33 H100) whose **mechanism is the VRAM concurrency ceiling, not compute overlap** (§9.3, §11.3) — the zero-parameter architectural knee is conservative (~2× low) and α closes the gap; α's *value* is an empirical per-GPU summary, exact only where KV fits VRAM. **Do NOT call α "compute overlap" — the derivation refutes that** (decode is 4.6%-compute, memory-bound throughout).
- The bandwidth model fits **85% of curves (82/96 at R²≥0.7)**; it breaks on the four H100/d512/ctx32000 non-monotone curves where the scheduler wave-batches above the VRAM ceiling — state "fits the regime where KV fits VRAM," not "fits all curves."
